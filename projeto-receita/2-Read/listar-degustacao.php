...
try {
    $stmt = $conexao->prepare("SELECT D.*, Funcionario.nome AS nomeDegustador, C.nome AS nomeCozinheiro, R.nome AS nomeReceita
                                FROM Degustacao D
                                ...
                                INNER JOIN Receita R ON D.nome = R.id_receita");
    $stmt->execute();
...
<body>
    ...
    <table>
        ...
        <?php foreach ($degustacoes as $degustacao): ?>
            <tr>
                <td><?= $degustacao['id_degustacao'] ?></td>
